...
#include "./data/stella_background.h"
#include "./data/stella_load.h"
#include "./data/stella_skin.h"
...
void screen_prepbackground(void) {
	...
	screen_prepback(layerbackgrey, STELLA_BACKGROUND, STELLA_BACKGROUND_SIZE);
}
...
void screen_showtopmenu(void) {
	...
	if (!GameConf.m_ScreenRatio) {
		screen_prepback(actualScreen, STELLA_SKIN, STELLA_SKIN_SIZE);
		SDL_Flip(actualScreen);
		screen_prepback(actualScreen, STELLA_SKIN, STELLA_SKIN_SIZE);
		SDL_Flip(actualScreen);
	}
...
signed int load_file(char **wildcards, char *result) {
	...
	while (return_value == 1) {
		...
		while(repeat) {
			...
			screen_prepback(layer, STELLA_LOAD, STELLA_LOAD_SIZE);
...
#define CHARLEN ((320/6)-2)
			...
			// Right - arrow down from current screen
			if (keys[SDLK_RIGHT] == SDL_PRESSED) { 
				if (!keyr) {
					keyr = 1;
			...
			// Left - arrow up from current screen
...
char *file_ext[] = { (char *) ".a26", (char *) ".bin", NULL };
